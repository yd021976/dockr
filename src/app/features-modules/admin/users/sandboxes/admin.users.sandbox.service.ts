...
export class AdminUsersSandboxService extends AdminUsersSandboxInterface {
    ...
    private loadAllUsers(): Promise<void> {
        ...
        return this.users_service.find()
            ...
            .catch(err => {
                /** reset users state to defaults, dispatch error and notify application of error */
                this.store.dispatch([
                    new Users_Load_All_Error(err.message),
                    new StateReset(UsersState),
                    new ApplicationNotifications_Append_Message(new ApplicationNotification(err.message, 'LoadAllUsers', ApplicationNotificationType.ERROR))]
                )
            })
    }
    ...
    private loadAllRoles(): Promise<void> {
        ...
        return this.roles_service.find()
            ...
            .catch(err => {
                this.store.dispatch([
                    new RolesStateActions.Load_All_Error(err.message)]
                )
            })
    }
    /**
